...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    const int delta_v_cost_weight = 500;
    ...
    for (t = 0; t < N - 1; t++) {
      ...
      fg[0] += delta_v_cost_weight * CppAD::pow(vars[delta_start + t] * vars[v_start+t], 2); // prevent llarge delta at high speed
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // std::cout << "Predict: " << pred_x << "; " << pred_y << "; " << pred_psi << "; " << pred_v << "; " << pred_cte << "; " << pred_epsi << "; " << std::endl;
